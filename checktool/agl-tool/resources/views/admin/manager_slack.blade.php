                    ...
                    <table class="table table-chatwork_translate_v2">
                        <thead>
                            <tr>
                                ...
                                <th style="border: 1px solid #ccc">Total (character)</th>
                        ...
                        <tbody>
                            ...
                            <tr id="{{ $data['team']->id }}" class="team">
                                        ...
                                        <li style="border-bottom: 1px solid #ccc; list-style: none; padding: 3%;">
                                            <a target="_blank" href="https://app.slack.com/client/{{ $data['team']->id }}/{{ $group_id }}/details/apps">
                                                {{$data['conversations_info'][$group_id]->name}}
                                                  <span style="font-size: 0.8rem; font-weight: normal; color: black">( {{ $data['log'][$group_id] }} characters )</span>
                                            </a>
                                            <form method="GET" action="{{route('slack_translate_v1_edit_lang_admin')}}">
                                ...
                                <td style="border: 1px solid #ccc; text-align: center;">
                                    @php $total = 0; @endphp
                                    @foreach($data['groups'] as $group_id => $value)
                                        @php $total +=  $data['log'][$group_id]; @endphp
                                    @endforeach  
                                    {{ $total }}
                                </td>
                                <td style="border: 1px solid #ccc; text-align: center;">
                                    <form method="GET" action="{{route('slack_translate_v1_delete_admin')}}">
                                        <input type="hidden" name="team_id" value="{{ $data['team']->id }}">
