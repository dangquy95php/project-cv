<x-header>
    <x-slot name="title">{タグ名}｜日本ペイント株式会社</x-slot>
    <x-slot name="description">「{タグ名}」の用語一覧です。ニッペラボの用語解説では、塗料・塗装の専門用語や関連する用語の意味をわかりやすく解説します。</x-slot>
    <x-slot name="keywords">keywords, keywords</x-slot>
    <x-slot name="slug">term-tag</x-slot>
    <x-slot name="unique">nippelab_term</x-slot>
    <x-slot name="og_type">article</x-slot>
    <x-slot name="url"></x-slot>
</x-header>

<main class="p-term-tag">
    <section class="p-term-tag1">
        <div class="l-cont">
            <h1 class="c-title2 c-title2__style2">{{$tag_name->name}}<span>／用語解説</span></h1>
