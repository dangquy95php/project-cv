<x-header>
    <x-slot name="title">{{ $selected_system->name }}｜塗料系統で探す｜重防食用塗料｜日本ペイント株式会社</x-slot>
    <x-slot name="description">日本ペイント株式会社の「重防食用塗料／{{ $selected_system->name }}」製品一覧ページです。製品詳細をご覧いただけます。</x-slot>
    ...
    <x-slot name="url">https://www.nipponpaint.co.jp/products/large/paint/system/{{ $selected_system->name }}/</x-slot>
</x-header>

<main class="p-systemDetail">
    <div class="c-breadcrumb">
        <div class="l-cont">
            <ul>
                ...
                <li><span>{{ $selected_system->name }}</span></li>
            </ul>
        </div>
    </div>

    <section class="p-systemDetail1">
        <div class="l-cont">
            <h1 class="c-title2 c-title2__style2">{{ $selected_system->name }}<span>／ 塗料系統で探す／ 重防食用塗料</span></h1>
