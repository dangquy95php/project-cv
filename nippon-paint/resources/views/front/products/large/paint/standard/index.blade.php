<x-header>
    <x-slot name="title">{{ $selected_standard->standard_name }}｜製品規格から探す｜重防食用塗料｜日本ペイント株式会社</x-slot>
    <x-slot name="description">日本ペイント株式会社の「重防食用塗料」製品検索ページです。規格から製品を絞り込むことができます。</x-slot>
    <x-slot name="keywords">keywords, keywords</x-slot>
    <x-slot name="slug">{{ $selected_standard->slug }}｜製品規格から探す｜重防食用塗料｜日本ペイント株式会社</x-slot>
...
<main class="p-standardDetail">
    <div class="c-breadcrumb">
        <div class="l-cont">
            <ul>
                ...
                <li><span>[{{ $selected_standard->p_large_standard_category->name }}] {{ $selected_standard->standard_name }}</span></li>
            </ul>
        </div>
    </div>
    ...
    <section class="p-standardDetail1">
        <div class="l-cont">
            <h2 class="c-title2 c-title2__style2">[{{ $selected_standard->p_large_standard_category->name }}] {{ $selected_standard->standard_name }}
                <span>／ 製品規格から探す ／ 重防食用塗料</span></h2>
