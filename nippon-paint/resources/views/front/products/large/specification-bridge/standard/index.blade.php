<x-header>
    <x-slot name="title">[{{ $standard->p_large_standard_category->name }}] {{ $standard->standard_name }}から探す｜橋梁・コンクリート｜仕様を探す｜日本ペイント株式会社</x-slot>
    <x-slot name="description">日本ペイント株式会社の「重防食用塗料／橋梁」の製品仕様一覧ページです。規格「[{{ $standard->p_large_standard_category->name }}] {{ $standard->standard_name }}」に該当する製品の一覧です。</x-slot>
    ...
    <x-slot name="url">https://www.nipponpaint.co.jp/products/large/specification-bridge/standard/{{ $standard->slug }}/</x-slot>
</x-header>
...
<main class="p-bridge">
    ...
    <section class="p-bridge1">
        <div class="l-cont">
            <h2 class="c-title2 c-title2__style2">[{{ $standard->p_large_standard_category->name }}] {{ $standard->standard_name }}から探す<span>／ 橋梁・コンクリート ／ 仕様を探す</span></h2>
            <div class="c-block2">
                ...
                @include('front.elements.products.large_spec.spec_bridge')
                @endforeach
