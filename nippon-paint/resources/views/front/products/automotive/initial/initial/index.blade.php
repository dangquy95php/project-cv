<x-header>
    <x-slot name="title">{{ $p_automotive_search->initial }}行｜自動車補修用塗料｜日本ペイント株式会社</x-slot>
    <x-slot name="description">日本ペイント株式会社の自動車補修用塗料の「{{ $p_automotive_search->initial }}行」の製品一覧ページです。上塗塗料、クリヤー、プラサフ、パテなど豊富なラインナップでお客様のあらゆるニーズにお応えする日本ペイントの自動車補修用製品です。
    </x-slot>
    <x-slot name="keywords">keywords, keywords</x-slot>
    <x-slot name="slug">initial</x-slot>
    <x-slot name="unique">products_index</x-slot>
    <x-slot name="og_type">article</x-slot>
    <x-slot name="url">https://www.nipponpaint.co.jp/products/automotive/initial/{{ $p_automotive_search->initial }}/</x-slot>
</x-header>

<main class="p-initial">
    <div class="c-breadcrumb c-breadcrumb__style1">
        <div class="l-cont">
            <ul>
                ...
                <li><span>自動車補修用塗料{{ $p_automotive_search->initial }}行</span></li>
            </ul>
        </div>
    </div>

    <section class="p-initial1">
        <div class="l-cont">
            <h1 class="c-title2 c-title2__style2">{{ $p_automotive_search->initial }}行<span>／ 自動車補修用塗料</span></h1>
    ...
    <div class="p-initial2">
        <div class="l-cont">
            <div class="l-aside">
                ...
                <div class="l-sidebar">
                    @include('components.side4', ['parent_categories' => $p_automotive_search->subcategories])
                </div>
            </div>
        </div>
    </div><!-- / p-initial2 -->

    <section class="p-initial3">
        <div class="l-cont">
            <div class="c-block1 c-block1--style1">
                <div class="c-block1__form">
                    @include('front.elements.products.search.form', [
